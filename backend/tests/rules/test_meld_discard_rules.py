...
class TestGetInvalidDiscardTiles:
    def test_pong_delegates(self):
        ...
        result = get_invalid_discard_tiles(MeldType.PONG, [], tile)
        assert result == {tile}

    def test_gang_delegates(self):
        tile = Wind(WindType.DONG)
        result = get_invalid_discard_tiles(MeldType.GANG, [], tile)
        assert result == {tile}
    ...
    def test_returns_set(self):
        result = get_invalid_discard_tiles(MeldType.PONG, [], Suit(SuitType.DOT, 1))
